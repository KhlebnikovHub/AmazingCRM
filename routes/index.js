...
router.route('/').get(initUser, async (req, res) => {
  ...
  const delivered = await Orders.findAll({
    raw: true,
    where: { status_id: '3' },
  });
  const ready = await Orders.findAll({ raw: true, where: { status_id: '4' } });
  const document = await Orders.findAll({
    raw: true,
    where: { status_id: '5' },
  });
  const allStatus = await OrderStatus.findAll({ raw: true });

  const allStatusesCount = [
    awaitDelivery.length,
    inWork.length,
    delivered.length,
    ready.length,
    document.length,
